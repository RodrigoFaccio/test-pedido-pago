...
export default function Character({ character }: CharacterInterface) {
  ...
  return (
    <CharacterComponents character={character} params={`${router.query.id}`} />
  );
}
...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  const response = await api.get<CharactersInterface>(
    `/comics?limit=25&ts=${timestamp}&apikey=${public_key}&hash=${hash.hex()}`
  );
...
export async function getStaticProps({ params }: paramsProps) {
  ...
  const { data } = await api.get(
    `/comics/${params.id}?ts=1&apikey=${public_key}&hash=${hash.hex()}`
  );
